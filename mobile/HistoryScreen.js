...
export default function HistoryScreen() {
  ...
  const loadMonthData = async () => {
    ...
    try {
      ...
      const url = ${API_BASE}/api/entries/month//;
  ...
  const loadDayEntries = async (date) => {
    ...
    try {
      const token = await AsyncStorage.getItem('authToken');
      const res = await axios.get(${API_BASE}/api/entries/, { headers: { Authorization: Bearer  } });
 ...
 {week.map((day,j) => {
 const dateStr = day ? ${currentMonth.getFullYear()}-- : null;
 ...
 <MacrosPieChart protein={macroTotals.protein} carbs={macroTotals.carbs} fat={macroTotals.fat} size={100} strokeWidth={12} />
 </View>
